"""
Deterministic, domain-constrained repair of extracted field values.

This is what replaced the old multi-pass "read the page twice and compare"
extraction. That approach cost 2-3 vision calls per page and, crucially,
had no ground truth: it compared two guesses against each other, so two
passes making the SAME misread produced a confidently wrong value, and the
tie-break pass was simply a third guess.

This module checks values against things we actually know for certain from
config/field_definitions.json and config/form_types.json:

  - An Omang / Birth Certificate number is exactly 9 digits.
  - A bank branch code is exactly 6 digits.
  - An enum field's value must be one of that field's listed options.
  - A fund name must be one of the real BIFM Unit Trust funds.

Where the model's read violates one of those, we can often repair it with
certainty rather than re-asking the model - "12345O789" is not a plausible
Omang, but it is unambiguously "123450789" once the classic O/0 OCR
confusion is undone. That costs nothing, takes no time, and is
deterministic and auditable.

DELIBERATELY CONSERVATIVE. Every repair here either produces a value that
provably satisfies its constraint, or makes no change at all:
  - Digit repair never pads or truncates to reach the expected length; it
    only substitutes known letter/digit look-alikes, and only accepts the
    result if it lands on exactly the required number of digits.
  - Enum/fund snapping only accepts an unambiguous match (exactly one
    candidate). A value close to two options is left alone for a human.
Anything it can't repair is returned untouched and flagged, so the caller
(app.utils.field_validators) can penalise its confidence and surface it
for review - never silently "corrected" into a plausible-looking lie.

Scope note: this helps CONSTRAINED fields (IDs, enums, funds). It does
nothing for free text (names, addresses, employer) - there is no domain to
check those against, so their confidence rests on the model's own score
plus the shape checks in app.utils.field_validators.
"""
