"""
Google Gemini backend - alternative cloud LLM backend to Groq (the
default). No local download, no model warm-up.
...
dashboard) comfortably cover POC-scale batches. If a project's free-tier
quota is 0 (common on newer/region-restricted projects), every call fails
fast with a clear "enable billing" message rather than silently retrying a
request that can never succeed - see _parse_429 below.

Uses Gemini's generateContent REST endpoint directly via `requests` (no
extra SDK dependency needed). Exposes the same public interface as
app.llm.groq_client (ask_text, ask_vision, parse_json_response,
...
def _parse_429(resp: "requests.Response") -> tuple[bool, float]:
    """
    Inspects a 429 response body and decides:
      - is_hard_quota: True if any quota metric has limit=0, meaning this
        key/project has NO free-tier allowance at all. Retrying can never
        succeed in that case, so the caller should fail fast instead of
        burning the whole retry budget on a request that will never land.
            ...
            if resp.status_code == 429:
                ...
                if is_hard_quota:
                    ...
                    raise GeminiError(
                        ...
                        "or Google disabling no-billing access on newer "
                        "projects). This will not resolve by retrying. Fix by "
                        "either: (1) enabling billing on the Google Cloud "
                        "project behind this key at "
                        "https://aistudio.google.com/apikey, or (2) setting "
                        "LLM_PROVIDER=groq to use Groq's free tier instead."
                    )
...
def ask_vision(prompt: str, image_path: Path, json_mode: bool = True) -> LLMResponse:
    ...
    body: dict = {
        ...
        "generationConfig": {
            # Deterministic decoding for transcription, matching the Groq
            # backend's settings - sampling noise just adds inconsistent
...
def parse_json_response(response_text: str) -> dict:
    """Same defensive JSON extraction as the Groq backend - kept identical
